 ...
 * </ReferenceManyField>
 */
export const ReferenceManyField = ({ children, render, ...props }) => {
    if (children && render) {
        throw new Error(
            'Both children and render prop supplied to <ReferenceManyField> only one is allowed'
        );
    }
    if (React.Children.count(children) !== 1 && !render) {
        throw new Error(
            '<ReferenceManyField> only accepts a single child (like <Datagrid>) or a render prop'
        );
    }
