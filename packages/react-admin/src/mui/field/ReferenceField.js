 ...
 * </ReferenceField>
 */
const ReferenceField = ({ children, render, ...props }) => {
    if (children && render) {
        throw new Error(
            'Both children and render prop supplied to <ReferenceField> only one is allowed'
        );
    }
    if (React.Children.count(children) !== 1 && !render) {
        throw new Error(
            '<ReferenceField> only accepts a single child or a render prop'
        );
    }
