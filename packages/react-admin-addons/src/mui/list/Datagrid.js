...
import DatagridHeaderCell from 'react-admin/lib/mui/list/DatagridHeaderCell';
import DatagridBody from 'react-admin/lib/mui/list/DatagridBody';
...
class Datagrid extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        const { ids, selectedIds } = this.props;
        if (nextProps.ids !== ids || nextProps.selectedIds !== selectedIds) {
            this.setState({
                selectAll:
                    nextProps.ids.length > 0 &&
                    !nextProps.ids.find(
                        id => nextProps.selectedIds.indexOf(id) === -1
                    ),
            });
        }
    }
